...
#its values into the defined train graph
        ...
        if y == 1:
            if str(d[x]) == "inf":
                print(item[0] + "--" + item[1]+ ": Can not close as there is no alterntive way between these stations")
            elif int(d[x]) >= 20:
                print(item[0] + "--" + item[1]+ ": Can not close other alterntive routes are unrealistically longer")
            else:
                print(item[0] + "--" + item[1]+ ": Can close as the alternitive journy is nominal")

#final overall output priting the starting station, the total time of the journy and the sstations list from before
#included slection outputs for the train clousser from task 4


histogramDataA = []
histogramDataB = []
plt.hist(histogramDataB, color = "red", bins=400,label='Before Closure')
plt.hist(histogramDataA, color = "blue", bins=400,label='After Closure')
#setting the histogram legend before hang
...
for item in edges:
        ...
        for i in range(len(vertices)):
                histogramDataB.append(d[i])

        lineClosure = (start, finish)
        if lineClosure in [item[:2] for item in edges]:
            edges = [item for item in edges if item[:2] != lineClosure]
            lineClosure = (finish, start)
            edges = [item for item in edges if item[:2] != lineClosure]
            edges = tuple(edges)
            ...
            for i in range(len(vertices)):
                histogramDataA.append(d[i])
            histogramDataA = [x for x in histogramDataA if x != float("inf")]
            histogramDataB = [x for x in histogramDataB if x != float("inf")]
            plt.xlabel('Time (Minuets)')
            plt.ylabel('Frequency')
            plt.title('Travel Times')
            plt.hist(histogramDataB, color = "red", bins=400)
            plt.hist(histogramDataA, color = "blue", bins=400)
...
#the starting input values that ask the user for thier starting station and thier desired destinaion, this
#value is used later in the actual logicn of the program plus a function to get both histogram data

#in this task we are asked to remove any connecctiosn to see if travel remains viable, there is an extra working
#here that removes the connection between the user input stations and outputs how travel is affected in the function
#later, this is designed for govermnet usage to input the two stations they are planning to close
